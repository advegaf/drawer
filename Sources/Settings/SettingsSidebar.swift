...
struct SettingsSidebar: View {
    ...
    /// The icon is inset 16pt while the rows below are inset 8pt, so the
    /// icon's leading edge sits 8pt inside the selection pills rather than
    /// flush with them. Flush reads as a list item that lost its pill.
    /// 32pt, not Wave's 24. Two reasons, both about this icon rather
    /// than about taste: the artwork is drawn per size, and at 24pt macOS
    /// reaches for the 32 pixel export, which is a denser glyph than the
    /// large one and read as a smudge against the sidebar. At 32 it picks
    /// the 64 and the drawer in it is legible.
    ///
    ...
    private var footer: some View {
        VStack(spacing: SettingsStyle.s4) {
            ...
            Text("\u{00A9} 2026 Angel Vega. All rights reserved.")
