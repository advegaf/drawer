...
final class EdgeCrossfadeTests: XCTestCase {
    ...
    func testItComesBackOnTheNewEdgeAtFullStrength() {
        ...
        XCTAssertEqual(controller.panelFrameForTesting?.minX ?? -1,
                       screen.visibleFrame.minX, accuracy: 1,
    ...
    func testOnlyTheLastEdgeAskedForWins() {
        ...
        // Left first and right second, rather than the same pair the other
        // way round: the notch starts on the right, so ending on the left is
        // a move that actually had to happen.
